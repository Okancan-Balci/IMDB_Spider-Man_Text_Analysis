...
title_bigrams <- review_data %>% 
  mutate(Title = str_to_lower(Review, locale = "en")) %>% 
...
```{r}
title_trigrams <- review_data %>% 
  mutate(Title = str_to_lower(Review, locale = "en")) %>% 
  unnest_tokens(output = Trigram, input = Title, token = "ngrams", n = 3, to_lower = FALSE)

filtered_title_trigrams <- title_trigrams %>% 
...
unigram_tokens %>% 
  ...
  geom_smooth(color = "darkred", size = 1.7) +
  facet_wrap(~Movie, scales = "free")
```
