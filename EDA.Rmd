...
review_data <- review_data %>% 
  mutate(Date_Binned = cut(Date, "months")) 
...
unigram_title_tokens %>% 
  ...
  filter(!Word %in% c("spiderman", "spider", "movie", "film")) %>% 
  group_by(Movie) %>% 
...
```{r}
filtered_title_bigrams <- review_data %>% 
...
```{r}
filtered_title_trigrams <- review_data %>% 
  mutate(Title = str_to_lower(Review, locale = "en")) %>% 
...
LDA_Data <- unigram_tokens %>% 
  anti_join(stop_words, by = c("Word" = "word")) %>% 
  filter(!Word %in% c("spider", "movie", "spiderman", "film")) %>% 
  group_by(Movie, Word) %>% 
