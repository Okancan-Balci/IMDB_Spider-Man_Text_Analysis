...
```{r}
# Reading the Data
review_data <- read_csv("imdb-spider-man-reviews.csv", show_col_types = FALSE)
...
review_data %>% 
  ...
  geom_boxplot()+
  geom_smooth(aes(Rating, Review_Length), inherit.aes = F, size = 1.3, color = "darkblue")+
  facet_wrap(~Movie)+
  scale_x_continuous(n.breaks = 10)+
  scale_y_log10()+
...
review_data %>% 
  ...
  geom_boxplot()+
  geom_smooth(inherit.aes = FALSE, aes(Rating, Helpfulness))+
  facet_wrap(~Movie)+
  scale_x_continuous(n.breaks = 10)
```
...
review_data %>% 
  ...
  geom_vline(xintercept = ymd("2021-12-15"), color = "black", size = 1.3)+
  geom_vline(xintercept = ymd("2018-02-18"), color = "red", size = 1.3)+# Bully Maguire First Appearance 
  geom_smooth(color = "darkred", size = 1.6)+
...
review_data %>% 
  ...
  geom_vline(xintercept = ymd("2021-12-15"), color = "red", size = 1.1, alpha = 0.8)+
  geom_vline(xintercept = ymd("2018-02-18"), color = "darkred", size = 1.3)+
  facet_wrap(~Movie, scales = "free")+
...
* The spider-man in the movie Into the Spider-Verse is Miles Morales that's why the word "*peter*" isn't the first of the second frequent word.
...
filtered_bigrams %>% 
  ...
  filter(Bigram_Count > 75) %>% 
  graph_from_data_frame() %>% 
  ggraph(layout = "fr")+
  geom_edge_link(aes(edge_width = Bigram_Count), edge_color = "steelblue", edge_alpha = 0.8,
                 arrow = arrow(length = unit(4, 'mm')))+
  geom_node_point(size = 5, alpha = 0.4)+
  geom_node_text(aes(label = name), repel = T, point.padding = unit(0.2, "lines"))+
  theme_void()
```

### Graphs for every other movie

```{r}
plot_movie_graphs <- function(Movie_Name) {
...
filtered_bigrams %>% 
  ...
  count(sort = T, name = "Bigram_Count") %>% 
  separate(col = Bigram, into = c("first", "second"), sep = " ") %>% 
  filter(Bigram_Count > (mean(Bigram_Count) * 40)) %>% 
  graph_from_data_frame() %>% 
  ggraph(layout = "fr")+
  geom_edge_link(aes(edge_width = Bigram_Count), edge_color = "steelblue", edge_alpha = 0.8,
                 arrow = arrow(length = unit(4, 'mm')))+
...
Movie_Names <- review_data %>% select(Movie) %>% unique() %>% pull(Movie)

map(Movie_Names, plot_movie_graphs)
```
...
Kind of similar to the unigram counts we analyzed above. Let's now plot TF-IDF for each movie.
...
```{r}
afinn_filtered <- get_sentiments("afinn") %>% 
...
unigram_tokens %>% 
  ...
  theme(strip.text.y = element_text(angle = 360),
        strip.background = element_rect(color = "darkred", fill = NA))+
    labs(y = NULL, x = "Total Sentiment")
```
...
We might expect a positive relationship between the helpfulness of a review and and it's sentiment score because the way a review is written might change how it is received by readers. A more positively written review might attract more helpful votes even tough the reader doesn't agree with that particular review.
...
unigram_tokens %>% 
  ...
  geom_smooth(color = "red", size = 1.4) +
  facet_wrap(~Movie, scales = "free")+
  theme(strip.background = element_rect(color = "darkred", fill = NA))+
...
Let's print some worst reviews for each movie according to my arbitrary conditions.

```{r}
print_reviews(worst_reviews$Review_ID)
```

The review of Spider-Man is a false positive here unfortunately. Looks like the reviewer only summarized the movie and because lots of bad stuff happens in an action superhero movie the review on the average got a negative sentiment score.He/she even rated 9! Other reviews make sense, they are indeed negative and their ratings score are very low. Note that there wasn't any review for Spider-Man 2 which suited my conditions.
...
* The word "*regret*" used with *won't* most of the time for movies No Way Home and Spider-Man 2 and Into the Spider-Verse.

* "*like*" was negated too many times with "*dont*".
...
```{r}
Review_N <- unigram_tokens %>% 
  group_by(Movie) %>% 
  count(name = "Total_Word")

unigram_tokens %>% 
  inner_join(get_sentiments("nrc") %>% filter(word != "spider"), by = c("Word" = "word")) %>% 
  group_by(Movie, sentiment) %>% 
  count() %>% 
  inner_join(Review_N, by = "Movie") %>% 
  mutate(Emotion_Proportion = n / Total_Word) %>% 
  ggplot(aes(Emotion_Proportion, reorder_within(sentiment, Emotion_Proportion, Movie), fill = Movie))+
  geom_col(show.legend = F)+
  facet_wrap(~Movie, scales = "free_y")+
  scale_y_reordered()+
  labs(y = NULL, x = "Emotion Proportion")
...
2 - MCU
3 - TASM
4 - Spider-Verse & OG Trilogy

```{r}
tidy(movie_LDA_4, matrix = "gamma") %>% 
  ggplot(aes(gamma, reorder_within(document, gamma, topic), fill = as.factor(topic)))+
  geom_col(show.legend = F)+
